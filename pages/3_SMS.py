...
async def main():
	# DB init from classes file
	SQLite = Database(db_name='app-db', url='database/app.db')

	# Initiliaze validation logic vars
	...
	st.header('SMS Distribution', divider='rainbow')
	...
	st.subheader('1. Upload a Recipient CSV file')
	...
	if distro is True:
		# Interesting - st.stop -> kill switch?
		try:
			msg_df = await asyncio.wait_for(distribute_sms(df), timeout=3)
			distro_success = True
		except asyncio.TimeoutError:
